...
from nac_triplet_relaxation import (
    ...
    RESIDUES_PER_CHAIN,
    SCRIPT_DIR,
    build_corresponding_contact_masks,
    ...
    corresponding_residue_contacts_from_close_pairs,
    crossing_time,
    get_frame_iterator,
    get_valid_box,
...
NAC_ACF_CACHE_CSV = "nac_triplet_acf_data.csv"
NAC_ACF_SUMMARY_CSV = "nac_triplet_acf_summary.csv"
NAC_ACF_TRAJ_SUMMARY_CSV = "nac_triplet_acf_trajectory_summary.csv"
    ...
    for frame_idx, ts in enumerate(get_frame_iterator(universe, start_frame, end_frame, skip_frame)):
        ...
        if contact_mode == "any_nac_to_nac_bb":
            ...
            for pair_start in range(0, n_pairs, pair_chunk):
                ...
                valid_ids = full_to_valid_pair[pair_start:pair_end]
                write_mask = valid_ids >= 0
                ...
                contact_series[valid_ids[write_mask], frame_idx] = contact_mask[write_mask]
    ...
    for frame_idx, ts in enumerate(get_frame_iterator(universe, start_frame, end_frame, skip_frame)):
        ...
        if needs_any:
            ...
            for pair_start in range(0, n_valid_pairs, pair_chunk):
                ...
                for contact_mode, contact_mask in masks.items():
                    contact_series_by_mode[contact_mode][pair_start:pair_end, frame_idx] = contact_mask
...
def analyze_single_nac_contact_acf_trajectory_multi(
    tpr_file,
    xtc_file,
    start_frame,
    end_frame,
    ...
    results = {}
    for contact_mode, contact_data in contact_data_by_mode.items():
        mode_cfg = get_contact_mode_config(contact_mode)
        mode_max_lag_frames = max_lag_frames
        if mode_max_lag_frames is None and max_lag_ns is not None:
            mode_max_lag_frames = int(np.floor(max_lag_ns * 1000.0 / contact_data["dt_ps"]))
        ...
        acf = compute_contact_acf(
            ...
            acf_kind=acf_kind,
            acf_pair_chunk=acf_pair_chunk,
            max_lag_frames=mode_max_lag_frames,
        )
        del contact_data["contact_series"]
        acf.update(contact_data)
        acf["acf_kind"] = acf_kind
...
def run_nac_contact_acf_analysis_multi(
    ...
    for contact_mode in contact_modes:
        ...
        if not force and os.path.exists(paths["cache_csv"]) and os.path.exists(paths["summary_csv"]):
            ...
            if not cache_filter_matches:
                print(f"Cached {contact_mode} ACF data use different chain filtering; recalculating.")
    ...
    for group in groups:
        ...
        print("=" * 80)
        print(f"Processing shared corresponding {acf_kind} ACF scan: {group_label}")
        ...
        for traj_idx, traj in enumerate(group["trajectories"], start=1):
            traj_results = analyze_single_nac_contact_acf_trajectory_multi(
                traj["tpr"],
                traj["xtc"],
                traj[start_key],
                end_frame,
            ...
            for contact_mode in modes_to_run:
                ...
                acf_lists[contact_mode].append(result["acf"])
                dt_hints[contact_mode].append(result["dt_ps"])
                traj_contact_observations[contact_mode].append(result["n_contact_observations"])
                traj_total_observations[contact_mode].append(result["n_observations"])
                traj_variable_pairs[contact_mode].append(result["n_variable_pairs"])
...
def run_nac_contact_acf_analysis(
    ...
    if not force and os.path.exists(cache_csv) and os.path.exists(summary_csv):
        ...
        if not cache_filter_matches:
            print("Cached NAC ACF data use different chain filtering; recalculating.")
        elif max_lag_ns is not None and not df_all.empty:
            cached_max_lag_ps = float(df_all["time_ps"].max())
            required_max_lag_ps = float(max_lag_ns) * 1e3
            if cached_max_lag_ps + 1e-9 < required_max_lag_ps:
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate NAC BB contact autocorrelation functions for the dense M3IDP trajectories used by dense.ipynb."
    )
    parser.add_argument(
        "--group",
        action="append",
    ...
    parser.add_argument(
        ...
        help="Base directory for per-mode output folders. Default: Analysis/NAC_contact_lifetime/NAC_contact_relaxation_time/nac_contact_acf_outputs.",
    )
    parser.add_argument(
        "--cutoff-nm",
        type=float,
    ...
    parser.add_argument(
        "--separate-modes",
        action="store_true",
        help="Run each contact mode with a separate trajectory scan, matching the legacy implementation.",
    )
    parser.add_argument(
        "--no-valid-chain-cache",
        action="store_true",
        help="Do not cache largest-cluster valid-chain masks on disk.",
    )
    parser.add_argument(
        "--valid-chain-cache-dir",
        default=DEFAULT_VALID_CHAIN_CACHE_DIR,
        help="Directory for cached largest-cluster valid-chain masks.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
