...
NAC_ANTISYMMETRIC_SUM_WINDOW = 8.0
...
NAC_CACHE_CSV = "nac_triplet_antisymmetric_survival_data.csv"
NAC_SUMMARY_CSV = "nac_triplet_antisymmetric_relaxation_summary.csv"
NAC_TRAJ_SUMMARY_CSV = "nac_triplet_antisymmetric_trajectory_summary.csv"
...
CONTACT_MODE_CONFIG = {
    "triplet_antisymmetric_nac_bb": {
        "label": "3 consecutive banded antisymmetric NAC BB",
        "observable": "NAC_triplet_antisymmetric_BB_contact_survival",
        "file_prefix": "nac_triplet_antisymmetric_bb",
        "event_unit": "chain_pair",
    },
    "antisymmetric_nac_bb": {
        "label": "Any 1 banded antisymmetric NAC BB",
        "observable": "NAC_antisymmetric_BB_contact_survival",
        "file_prefix": "nac_antisymmetric_bb",
        "event_unit": "chain_pair",
    },
    "three_antisymmetric_nac_bb": {
        "label": "Any 3 banded antisymmetric NAC BB",
        "observable": "NAC_three_antisymmetric_BB_contact_survival",
        "file_prefix": "nac_three_antisymmetric_bb",
        "event_unit": "chain_pair",
    },
    "any_nac_to_nac_bb": {
...
def resolve_antisymmetric_sum_target(nac_residue_range, antisymmetric_sum_target=None):
    if antisymmetric_sum_target is not None:
        return float(antisymmetric_sum_target)
    return float(nac_residue_range[0] + nac_residue_range[1])


def build_banded_antisymmetric_indices(
    nac_residue_range,
    ...
    target = resolve_antisymmetric_sum_target(
        nac_residue_range,
        antisymmetric_sum_target=antisymmetric_sum_target,
    )
    allowed = np.abs(residue_ids[:, None] + residue_ids[None, :] - target) < float(antisymmetric_sum_window)
    source_idx, partner_idx = np.where(allowed)
    if len(source_idx) == 0:
        raise ValueError(
            "No banded antisymmetric residue pairs found for "
            f"range={nac_residue_range}, target={target}, window={antisymmetric_sum_window}."
        )
    return source_idx.astype(np.int64), partner_idx.astype(np.int64), target
...
def build_banded_contact_masks(
    ...
    triplet_partner_direction="reverse",
):
    masks = {}
    if "antisymmetric_nac_bb" in contact_modes:
        masks["antisymmetric_nac_bb"] = np.any(residue_contacts, axis=1)
    if "three_antisymmetric_nac_bb" in contact_modes:
        masks["three_antisymmetric_nac_bb"] = np.sum(residue_contacts, axis=1) >= 3
    if "triplet_antisymmetric_nac_bb" in contact_modes:
        masks["triplet_antisymmetric_nac_bb"] = build_banded_triplet_contact_mask(
            residue_pair_contacts,
            n_triplets,
...
def run_nac_contact_relaxation_analysis_multi(
    ...
    for group in groups:
        ...
        print("=" * 80)
        print(f"Processing shared antisymmetric survival scan: {group_label}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate antisymmetric NAC BB contact survival times for the dense M3IDP trajectories."
    )
    parser.add_argument(
        "--group",
        action="append",
