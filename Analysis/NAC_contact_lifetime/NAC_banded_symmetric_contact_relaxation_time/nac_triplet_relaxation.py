...
NAC_SYMMETRIC_WINDOW = 8.0
BANDED_TRIPLET_DEFINITION = "partner_consecutive_v2"
...
NAC_CACHE_CSV = "nac_triplet_symmetric_survival_data.csv"
NAC_SUMMARY_CSV = "nac_triplet_symmetric_relaxation_summary.csv"
NAC_TRAJ_SUMMARY_CSV = "nac_triplet_symmetric_trajectory_summary.csv"
...
CONTACT_MODE_CONFIG = {
    "triplet_symmetric_nac_bb": {
        "label": "3 consecutive banded symmetric NAC BB",
        "observable": "NAC_triplet_symmetric_BB_contact_survival",
        "file_prefix": "nac_triplet_symmetric_bb",
        "event_unit": "chain_pair",
    },
    "symmetric_nac_bb": {
        "label": "Any 1 banded symmetric NAC BB",
        "observable": "NAC_symmetric_BB_contact_survival",
        "file_prefix": "nac_symmetric_bb",
        "event_unit": "chain_pair",
    },
    "three_symmetric_nac_bb": {
        "label": "Any 3 banded symmetric NAC BB",
        "observable": "NAC_three_symmetric_BB_contact_survival",
        "file_prefix": "nac_three_symmetric_bb",
        "event_unit": "chain_pair",
    },
    "any_nac_to_nac_bb": {
...
def build_banded_symmetric_indices(
    nac_residue_range,
    symmetric_window=NAC_SYMMETRIC_WINDOW,
):
    nac_start, nac_end = nac_residue_range
    residue_ids = np.arange(nac_start, nac_end + 1, dtype=float)
    allowed = np.abs(residue_ids[:, None] - residue_ids[None, :]) < float(symmetric_window)
    source_idx, partner_idx = np.where(allowed)
    if len(source_idx) == 0:
        raise ValueError(
            "No banded symmetric residue pairs found for "
            f"range={nac_residue_range}, window={symmetric_window}."
        )
    return source_idx.astype(np.int64), partner_idx.astype(np.int64)
...
def build_banded_allowed_matrix(n_residues, source_idx, partner_idx):
    allowed = np.zeros((n_residues, n_residues), dtype=bool)
    allowed[source_idx, partner_idx] = True
    return allowed


def banded_residue_contacts_from_close_pairs(
    close_pairs,
    flat_chain_ids,
    flat_residue_idx,
    pair_index,
    ...
    n_valid_pairs,
    n_residues,
    return_pair_contacts=False,
):
    residue_contacts = np.zeros((n_valid_pairs, n_residues), dtype=bool)
    residue_pair_contacts = (
        np.zeros((n_valid_pairs, n_residues, n_residues), dtype=bool)
        if return_pair_contacts
        else None
    )
    if len(close_pairs) == 0:
        if return_pair_contacts:
            return residue_contacts, residue_pair_contacts
    ...
    band_keep = band_allowed[source_residue, partner_residue]
    if not np.any(band_keep):
        if return_pair_contacts:
            return residue_contacts, residue_pair_contacts
        return residue_contacts

    band_source_residue = source_residue[band_keep]
    band_partner_residue = partner_residue[band_keep]
    full_pair_idx = pair_index[source_chain[band_keep], partner_chain[band_keep]]
    ...
    if np.any(valid):
        valid_pair_idx = valid_pair_idx[valid]
        valid_source_residue = band_source_residue[pair_valid][valid]
        residue_contacts[valid_pair_idx, valid_source_residue] = True
        if return_pair_contacts:
            valid_partner_residue = band_partner_residue[pair_valid][valid]
            residue_pair_contacts[valid_pair_idx, valid_source_residue, valid_partner_residue] = True
...
def build_banded_triplet_contact_mask(
    residue_pair_contacts,
    n_triplets,
    triplet_size=NAC_TRIPLET_SIZE,
    partner_direction="forward",
):
    if residue_pair_contacts is None:
        raise ValueError("residue_pair_contacts is required for banded triplet contact modes")

    n_pairs = residue_pair_contacts.shape[0]
    triplet_contacts = np.ones((n_pairs, n_triplets, n_triplets), dtype=bool)
    for offset in range(triplet_size):
        source_slice = slice(offset, offset + n_triplets)
        if partner_direction == "forward":
            partner_slice = slice(offset, offset + n_triplets)
        elif partner_direction == "reverse":
            partner_slice = slice(triplet_size - 1 - offset, triplet_size - 1 - offset + n_triplets)
        else:
            raise ValueError(f"Unknown partner_direction={partner_direction!r}")
        triplet_contacts &= residue_pair_contacts[:, source_slice, partner_slice]
    return np.any(triplet_contacts, axis=(1, 2))


def build_banded_contact_masks(
    residue_contacts,
    contact_modes,
    n_triplets,
    triplet_size=NAC_TRIPLET_SIZE,
    residue_pair_contacts=None,
    triplet_partner_direction="forward",
):
    masks = {}
    if "symmetric_nac_bb" in contact_modes:
        masks["symmetric_nac_bb"] = np.any(residue_contacts, axis=1)
    if "three_symmetric_nac_bb" in contact_modes:
        masks["three_symmetric_nac_bb"] = np.sum(residue_contacts, axis=1) >= 3
    if "triplet_symmetric_nac_bb" in contact_modes:
        masks["triplet_symmetric_nac_bb"] = build_banded_triplet_contact_mask(
            residue_pair_contacts,
            n_triplets,
            triplet_size=triplet_size,
            partner_direction=triplet_partner_direction,
        )
    return masks
...
def cache_matches_chain_filter(
    ...
    if summary_df.empty:
        return False
    if "banded_triplet_definition" not in summary_df:
        return False
    definitions = set(summary_df["banded_triplet_definition"].dropna().astype(str))
    if definitions != {BANDED_TRIPLET_DEFINITION}:
        return False
...
def analyze_single_nac_contact_trajectory(
    ...
    if contact_mode == "triplet_symmetric_nac_bb" and n_triplets <= 0:
        raise ValueError(f"NAC length {n_nac} is shorter than triplet_size={triplet_size}.")
    band_source_idx, band_partner_idx = build_banded_symmetric_indices(
        nac_residue_range,
    ...
    if contact_mode in {
        ...
        n_contact_sites = 1
        n_active_pairs = n_pairs if contact_mode == "any_nac_to_nac_bb" else n_valid_pairs
        active_lengths = np.zeros(n_active_pairs, dtype=np.int32)
    else:
    ...
    for frame_idx, ts in enumerate(get_frame_iterator(universe, start_frame, end_frame, skip_frame)):
        ...
        if contact_mode == "any_nac_to_nac_bb":
            ...
            needs_triplet_contacts = contact_mode == "triplet_symmetric_nac_bb"
    ...
    survival.update(
        {
            ...
            "symmetric_window": float(symmetric_window),
        }
    )
    return survival
...
def analyze_single_nac_contact_trajectory_multi(
    ...
    n_nac = nac_bb_indices.shape[1]
    n_triplets = n_nac - triplet_size + 1
    if "triplet_symmetric_nac_bb" in contact_modes and n_triplets <= 0:
        raise ValueError(f"NAC length {n_nac} is shorter than triplet_size={triplet_size}.")
    band_source_idx, band_partner_idx = build_banded_symmetric_indices(
        nac_residue_range,
        symmetric_window=symmetric_window,
    ...
    symmetric_modes = tuple(mode for mode in contact_modes if mode != "any_nac_to_nac_bb")
    active_lengths = {}
    for contact_mode in contact_modes:
        n_active_pairs = n_pairs if contact_mode == "any_nac_to_nac_bb" else n_valid_pairs
        active_lengths[contact_mode] = np.zeros(n_active_pairs, dtype=np.int32)
    ...
    for frame_idx, ts in enumerate(get_frame_iterator(universe, start_frame, end_frame, skip_frame)):
        ...
        if symmetric_modes:
            needs_triplet_contacts = "triplet_symmetric_nac_bb" in symmetric_modes
            contact_result = banded_residue_contacts_from_close_pairs(
                close_pairs,
                nac_flat_chain_ids,
                nac_flat_residue_idx,
...
def analyze_single_nac_triplet_trajectory(*args, **kwargs):
    kwargs["contact_mode"] = "triplet_symmetric_nac_bb"
...
def run_nac_contact_relaxation_analysis_multi(
    ...
    for contact_mode in contact_modes:
        ...
        if not force and os.path.exists(paths["cache_csv"]) and os.path.exists(paths["summary_csv"]):
            ...
            if cache_matches_chain_filter(
                ...
                continue
            print(f"Cached {contact_mode} survival data use different chain filtering or contact definition; recalculating.")
    ...
    for group in groups:
        ...
        print("=" * 80)
        print(f"Processing shared symmetric survival scan: {group_label}")
                ...
                traj_summary_rows_by_mode[contact_mode].append(
                    {
                        ...
                        "symmetric_window": result["symmetric_window"],
                    }
                )
...
def run_nac_contact_relaxation_analysis(
    ...
    if not force and os.path.exists(cache_csv) and os.path.exists(summary_csv):
        ...
        print("Cached NAC data use different chain filtering or contact definition; recalculating.")
...
def run_nac_triplet_relaxation_analysis(*args, **kwargs):
    kwargs.setdefault("contact_mode", "triplet_symmetric_nac_bb")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate symmetric NAC BB contact survival times for the dense M3IDP trajectories."
    )
    parser.add_argument(
        "--group",
        action="append",
    ...
    parser.add_argument(
        ...
        help="Contact definition to run. Can be used multiple times. Default: all symmetric NAC contact modes.",
    )
    parser.add_argument(
        "--output-dir",
        default=os.path.join(SCRIPT_DIR, "nac_contact_outputs"),
        help="Base directory for per-mode output folders. Default: this folder/nac_contact_outputs.",
    )
    parser.add_argument(
        "--cutoff-nm",
        type=float,
    ...
    parser.add_argument(
        "--symmetric-window",
        type=float,
        default=NAC_SYMMETRIC_WINDOW,
        help="Require abs(resid_i - resid_j) < this value. Default: 8.",
    )
    parser.add_argument(
        "--radius-limit-nm",
        type=float,
