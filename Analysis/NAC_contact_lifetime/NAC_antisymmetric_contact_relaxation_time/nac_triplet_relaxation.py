    ...
    for frame_idx, ts in enumerate(get_frame_iterator(universe, start_frame, end_frame, skip_frame)):
        ...
        if contact_mode == "any_nac_to_nac_bb":
            ...
            for pair_start in range(0, n_pairs, pair_chunk):
                ...
                elif contact_mode == "three_antisymmetric_nac_bb":
...
def analyze_single_nac_contact_trajectory_multi(
    ...
    active_lengths = {
        contact_mode: np.zeros(n_pairs, dtype=np.int32)
        for contact_mode in contact_modes
    }
    ...
    for frame_idx, ts in enumerate(get_frame_iterator(universe, start_frame, end_frame, skip_frame)):
        ...
        if antisymmetric_modes:
            for pair_start in range(0, n_pairs, pair_chunk):
                ...
                masks = {}
                if "antisymmetric_nac_bb" in antisymmetric_modes:
                    masks["antisymmetric_nac_bb"] = np.any(residue_contacts, axis=1)
                if "three_antisymmetric_nac_bb" in antisymmetric_modes:
                    masks["three_antisymmetric_nac_bb"] = np.sum(residue_contacts, axis=1) >= 3
                if "triplet_antisymmetric_nac_bb" in antisymmetric_modes:
                    triplet_contacts = np.ones((len(pair_block), n_triplets), dtype=bool)
                    for offset in range(triplet_size):
                        triplet_contacts &= residue_contacts[:, offset : offset + n_triplets]
                    masks["triplet_antisymmetric_nac_bb"] = np.any(triplet_contacts, axis=1)
